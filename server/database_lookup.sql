...
SELECT run_questions.*,
    questions.*
FROM run_questions
    JOIN runs ON run_questions.run_id = runs.id
    JOIN questions ON run_questions.question_id = questions.id
WHERE runs.id == 0
ORDER BY questions.difficulty ASC;