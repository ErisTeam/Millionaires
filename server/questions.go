...
func assignQuestionToRun(ctx *fiber.Ctx, runId Snowflake, questionId int32) error {
    ...
    _, err := db.Exec("INSERT INTO run_questions (run_id, question_id) VALUES (?, ?);", runId.ID, questionId)
    return err;
}
...
func answerQuestion(c *fiber.Ctx) error {
    ...
    runQuestionRow := db.QueryRow("SELECT run_questions.id, run_questions.answer_id, run_questions.answered_at FROM run_questions JOIN runs ON run_questions.run_id = runs.id WHERE run_questions.run_id = ? AND run_questions.question_id = (SELECT answers.question_id FROM answers WHERE answers.id = ?);", runId.ID, request.AnswerId)
	...
	if *answer.IsCorrect {
        ...
        if (*answer.Difficulty < int32(12)) {
            ...
            if err != nil {
                return c_error(c, fmt.Sprintf("Unable to assign the question to the run with id `%d`: `%s`", runId.ID, err), fiber.ErrInternalServerError.Code)
            }
